...
export const jobPreferencesSchema = Joi.object({
  ...
  countries: Joi.array().items(Joi.string().length(2).uppercase()).max(10).default([]),
  caProvinces: Joi.array().items(Joi.string().max(60)).max(13).default([]),
  usStates: Joi.array().items(Joi.string().max(60)).max(51).default([]),
  cities: Joi.array().items(Joi.string().max(80)).max(30).default([]),
  ...
  remoteTypes: Joi.array().items(Joi.string().valid('remote', 'hybrid', 'onsite')).max(3).default([]),
